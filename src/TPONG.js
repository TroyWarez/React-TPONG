...
function Draw()
{
  ...
  if (Ball.x <= window.innerWidth - 120 && Ball.x >= window.innerWidth - 90 && Ball.y < (CPUPaddle.y + 100) && Ball.y > CPUPaddle.y)
  {
    if(BallSpeedY < 0)
    {
      BallSpeedY = BallSpeedY * 1;
  ...
  if(gameFlags.StartGame === true)
  {
    ...
    if (CPUPaddle.y >= 0 && CPUPaddle.y <= (window.innerHeight - 100) && Ball.x >= (window.innerWidth / 2))
    {
      CPUPaddle.y = Ball.y;
    }
